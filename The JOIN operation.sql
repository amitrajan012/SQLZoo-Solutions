...
SELECT eteam.teamname, X.C
FROM eteam
JOIN ((select teamid as teamid, COUNT(player) as C
  FROM goal
  GROUP BY teamid) AS X)
ON (eteam.id = X.teamid)

/* 10. Show the stadium and the number of goals scored in each stadium.*/

SELECT game.stadium, COUNT(stadium)
FROM game JOIN goal
...
SELECT matchid, mdate, COUNT(gtime) AS count_goals
FROM game JOIN goal ON matchid = id
WHERE (teamid = 'GER')
GROUP BY matchid, mdate;
