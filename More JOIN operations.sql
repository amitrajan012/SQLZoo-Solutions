...
JOIN actor
ON (actor.id = casting.actorid)
WHERE casting.movieid IN ( SELECT movieid FROM casting WHERE actorid =
...
SELECT movie.title, COUNT(casting.actorid) as C
FROM movie JOIN casting
ON (movie.id = casting.movieid)
WHERE movie.yr = 1978
GROUP BY movie.title
ORDER BY C DESC, movie.title ASC;
...
FROM casting
WHERE casting.actorid = (SELECT id FROM actor WHERE name = 'Art Garfunkel'))
